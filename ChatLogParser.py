...
def grade_chat_log(content, host_name, known_questions, keywords, disallowed_phrases, triggers):
    ...
    for entry in content:  # Iterating through chat entries
        ...
        if(user == host_name):  # Checking if the user is the host
            ...
            if(trigger_present):  # If a trigger is already present
            ...
            continue
        if (trigger_present):  # If a question is currently active
...
# Placeholder for known questions and their correct answers
known_qs = {}
...
# Function to add a chat log to the program
def add_chat():
    global host  # Accessing the global variable host
    host = host_field.get()  # Getting host name from the input field
    if host == "":  # If host name is not provided
        ...
        if filepath:
            with open(filepath, 'rb') as f:
                result = chardet.detect(f.read())  # Detecting encoding of the file
                ...
                for line in file:
                    ...
                    if match:
                        ...
                        if "(Privately)" in user:
                            continue  # Skipping private messages
                        chat_entries.append({'user': user.strip(), 'message': message.strip()})  # Adding entry to the list
        ...
        if filepath:
            ...
            with open(filepath, 'r', encoding=encoding) as file:
                ...
                for line in file:
                    ...
                    print("afsfsdfaf"+line)
                    # Skip empty lines
                    if line.strip() == "":
                        continue
                    # Add each keyword from the file to the kw dictionary
                    kw[line.strip()]=weighting  # Missing code to handle keyword addition
# Function to add keywords for analysis
def add_quest():
    global host
    # Check if host's name is entered, show error if not
    if host == "":
        ...
        if filepath:
            ...
            with open(filepath, 'r', encoding=encoding) as file:
                ...
                kywrd_text.insert(tk.END, content)
                file.seek(0)
                # Loop through each line in the file to process questions and answers
                for line in file:
                    if line.strip() != "":  # Skip empty lines
                        continue
                    question, answer = line.strip().split(":")
                    known_qs[question] = answer
...
# Function to add questions and their answers for analysis
